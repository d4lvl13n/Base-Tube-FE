...
import { Video } from '../../types/video'; // Import the updated Video interface
...
const VideoCard: React.FC<VideoCardProps> = ({ video, size }) => {
  ...
  console.log('Thumbnail Path:', video.thumbnail_path);

  // Construct the thumbnail URL
  const thumbnailUrl = `${process.env.REACT_APP_API_URL}/${video.thumbnail_path}`;
  
  // Add console log for the full URL
      ...
      >
        ...
        {isHovered && (
          <div className="absolute bottom-0 left-0 right-0 p-4 text-white transition-all duration-300 ease-in-out transform translate-y-0">
            ...
            <p className="text-sm mb-2">{video.channel.name}</p>
            <div className="flex items-center space-x-4">
              <span className="flex items-center">
                <Play size={16} className="mr-1" /> {video.views}
              </span>
              <span className="flex items-center">
                <Heart size={16} className="mr-1" /> {video.likes || 0}
              </span>
              {/* Remove or adjust comments count if not available */}
            </div>
          </div>
        )}
      </div>
