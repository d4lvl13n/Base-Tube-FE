...
const StatCard: React.FC<StatCardProps> = ({ 
  ...
  <motion.div
    ...
    transition={{ delay }}
    className="bg-black/50 border border-gray-800/30 rounded-xl p-5 hover:bg-black/60 transition-colors backdrop-blur-sm"
  >
...
const AuditHistoryRow: React.FC<AuditHistoryRowProps> = ({ item, onView, delay = 0 }) => {
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      className="group bg-black/50 border border-gray-800/30 rounded-xl p-4 hover:bg-black/60 hover:border-gray-800/50 transition-all cursor-pointer backdrop-blur-sm"
      onClick={() => onView(item.id)}
    >
      <div className="flex items-center gap-4">
        {/* Thumbnail */}
        <div className="w-24 h-14 rounded-lg overflow-hidden bg-black/80 border border-gray-800/50 flex-shrink-0">
          {item.thumbnailUrl ? (
            <img 
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${scoreColorClasses[scoreColor]}`}>
              {score.toFixed(1)}/10
            </span>
          ...
          <div className="flex items-center gap-3 text-sm text-gray-400">
            <span className="capitalize font-medium text-gray-300">{item.detectedNiche}</span>
    ...
    <AIThumbnailsLayout quota={quota} isLoadingQuota={isLoadingQuota}>
      ...
      {auditHistory.length > 0 && (
        <motion.div
          ...
          className="bg-black/50 border border-gray-800/30 rounded-xl p-4 mb-6 backdrop-blur-sm"
        >
          <div className="flex items-center gap-4">
      ...
      <div className="space-y-3">
        ...
        {isLoadingAuditHistory && auditHistory.length === 0 && (
          <div className="space-y-3">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-black/50 border border-gray-800/30 rounded-xl p-4 animate-pulse backdrop-blur-sm">
                <div className="flex items-center gap-4">
                  <div className="w-24 h-14 rounded-lg bg-gray-800/50" />
                  <div className="flex-1">
        ...
        {!isLoadingAuditHistory && auditHistory.length === 0 && (
          ...
          >
            <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-black/50 border border-gray-800/30 flex items-center justify-center backdrop-blur-sm">
        ...
        {auditHistoryPagination?.hasMore && (
          ...
          >
            <button
              ...
              className="flex items-center gap-2 py-3 px-6 bg-black/50 hover:bg-black/60 border border-gray-800/30 hover:border-gray-800/50 rounded-xl text-gray-300 font-medium transition-all disabled:opacity-50 backdrop-blur-sm"
            >
              {isLoadingAuditHistory ? (
                <>
